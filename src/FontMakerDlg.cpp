...
/*
	...
	pComboBox->AddString(L"GBK");
	pComboBox->AddString(L"GB2312");
	pComboBox->AddString(L"ASCII");
	pComboBox->SetCurSel(0);
	count = 1;
	return count;
}
...
BOOL CFontMakerDlg::OnInitDialog()
{
	...
	SetDlgItemInt(IDC_EDIT_WIDTH,32);
	SetDlgItemInt(IDC_EDIT_HEIGHT,32);
	SetDlgItemInt(IDC_EDIT_HORZ,0);
	SetDlgItemInt(IDC_EDIT_VERT,0);
	SetDlgItemInt(IDC_EDIT_FONT_SIZE,35);
...
void CFontMakerDlg::OnDeltaposSpin6(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = 0;
	int total = m_charset.GetCharCount();
    if(pNMUpDown->iDelta > 0)
    {
        m_nCharIndex=(m_nCharIndex+1)% total;
    }
    else
    {
        m_nCharIndex=(m_nCharIndex + total-1)%total;
    }
    OnCharChange();
}
